<footer>
    <div class="container py-5">
        <div class="row">
            <div class="col-lg-3 col-md-4 col-sm-12">
                <div class="logo pb-3">
                    <a href="/"><img src="img/logo.png" alt=""srcset=""></a>
                </div>
                <div>
                    <p>Bienvenue dans l'une des meilleures expériences dentaires de votre région, où des soins de
                        qualité et un sourire éclatant vous attendent !</p>
                </div>
            </div>
            <div class="col-lg-3 col-md-4 col-sm-12">
                <h5>Bureau</h5>
                <li><a href=""><i class="fa-solid fa-location-dot"></i>Test , Test , N00</a></li>
                <li><a href=""><i class="fa-solid fa-envelope"></i>info@gmail.com</a></li>
                <li><a href=""><i class="fa-solid fa-phone"></i>+212 652583234</a></li>
            </div>
            <div class="col-lg-3 col-md-4 col-sm-12">
                <h5>Liens</h5>
                <li><a href="/" class="navigation">Accueil</a></li>
                <li><a href="/#à-propos" class="navigation">À propos</a>
                </li>
                <li><a href="/#services" class="navigation">Nos Service</a>
                </li>
                <li><a href="#home-contact" class="navigation">Contactez-nous</a></li>
                <li><a href="/appointment">Prenez rendez-vous en un clic</a></li>
            </div>
            <div class="col-lg-3 col-md-4 col-sm-12 media">
                <h5>Contactez-nous</h5>
                <li><a href=""><i class="fa-brands fa-whatsapp"></i>+212 652583234</a></li>
                <li><a href=""><i class="fa-brands fa-instagram"></i>test_test</a></li>
                <li><a href=""><i class="fa-brands fa-facebook"></i>Test Test</a></li>
                <li><a href=""><i class="fa-brands fa-telegram"></i>+212 652583234</a></li>
            </div>
        </div>
        <div class="pt-4 w-100">
            <p class="fs-6">Teljaoui Mohamed © 2024. Tous droits réservés.</p>
        </div>
    </div>

</footer>
